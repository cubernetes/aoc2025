...
circuits = {}
circuit_id = 1
#for distance in sorted_distances[:10]: # test
for distance in sorted_distances[:1000]: # real
    ...
    if p1 not in circuits and p2 not in circuits:
        ...
        dominant_id = circuits[p1]
        overwrite_id = circuits[p2]
...
circuits_grouped = defaultdict(list)
for point, circuit_id in circuits.items():
    circuits_grouped[circuit_id].append(point)
circuit_sizes = sorted(map(lambda tup: len(tup[1]), circuits_grouped.items()), reverse=True)
three_biggest = circuit_sizes[:3]

print(three_biggest)

print(eval('*'.join(map(str, three_biggest))))
